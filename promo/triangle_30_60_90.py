"""
triangle_30_60_90 — the "special" triangle everyone memorises. 40.0s.

    BPM=150 manimgl triangle_30_60_90.py Triangle306090 -w -r 1080x1920
...
ONE TRIANGLE, EVERY "SPECIAL ANGLE". A 30-60-90 triangle always has sides
in the exact ratio 1 : √3 : 2 — not approximately, exactly, no matter how
big or small it's drawn. Every "special angle" value from every trig class
comes straight off this one triangle:

        sin 30° = 1/2          cos 60° = 1/2
        sin 60° = √3/2         cos 30° = √3/2

sin 30° and cos 60° are the SAME ratio, off the SAME side — the cofunction
fact from two episodes ago, showing up again for free.
...
LONG = math.sqrt(3.0)
HYP = 2.0
assert abs(SHORT ** 2 + LONG ** 2 - HYP ** 2) < 1e-9
...
assert abs(SIN30 - COS60) < 1e-12
assert abs(SIN60 - COS30) < 1e-12
assert abs(COS30 - math.sqrt(3) / 2) < 1e-9
...
class Triangle306090(Scene):
    ...
    # ------------------------------------------------------------------
    def open_card(self):
        big = txt("1 : √3 : 2", 32, GOLD, w=4.5)
        big.move_to(np.array([0, 1.15, 0]))
        q = txt("the \"special\" triangle", 27, WHITE_, w=4.6)
        q.move_to(np.array([0, 0.15, 0]))
        sub = txt("every memorised value comes from here.", 22, GREY, bold=False)
        ...
        self.eq = txt("1 : √3 : 2", 30, GOLD, w=4.3)
    ...
    def stage_triangle(self):
        ...
        al = txt("30°", 19, GOLD, w=0.8).move_to(A + np.array([0.42, 0.16, 0]))
        cl = txt("60°", 19, SKY, w=0.8).move_to(C + np.array([-0.26, -0.34, 0]))
        shortl = txt("1", 20, WHITE_, w=0.4).move_to(
            (Bv + C) / 2 + np.array([0.26, 0, 0]))
        longl = txt("√3", 20, WHITE_, w=0.5).move_to(
            (A + Bv) / 2 + np.array([0, -0.28, 0]))
        hypl = txt("2", 20, ROSE, w=0.4).move_to(
            (A + C) / 2 + np.array([-0.30, 0.18, 0]))
        self.pic = VGroup(grid, edges, rt, al, cl, shortl, longl, hypl)

        self.play(FadeIn(grid), run_time=self.T(1))
        self.play(*[ShowCreation(e) for e in edges], run_time=self.T(2.5))
        self.play(FadeIn(rt), FadeIn(shortl), FadeIn(longl), FadeIn(hypl),
                  run_time=self.T(1.5))
        self.play(FadeIn(al), FadeIn(cl), run_time=self.T(1))
        self.say("30°, 60°, 90°. sides 1, √3, 2 — always.", 2.5, GOLD)
        self.pad_to(END_TRI)

    # ==================================================================
    def stage_values(self):
        self.say("sin 30°: opposite over hypotenuse.", 3, GOLD)
        self.set_work("sin 30° = 1/2", GOLD, 2.5)
        self.say("cos 60°: adjacent over hypotenuse. same side.", 3, SKY)
        self.set_work("cos 60° = 1/2", SKY, 2.5)
        self.say("sin 30° and cos 60° — the exact same ratio.", 3, GOLD)
        self.set_work("sin 60° = cos 30° = √3/2", GOLD, 3)
        self.say("the other two sides give the other two values.", 3.5)
        self.pad_to(END_SWAP)

    # ==================================================================
    def stage_why(self):
        self.say("one triangle. every special-angle value on the sheet.", 3)
        self.set_work("1 : √3 : 2 — memorise the triangle, not six numbers", GOLD, 3.5)
        self.pad_to(END_WHY)
